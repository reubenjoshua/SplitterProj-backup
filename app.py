        ...
        try:
                ...
                for atm_ref, transactions in processed_data.items():
                    ...
                    for trans in transactions:
                        if isinstance(trans, dict):
                            ...
                            # Extract dates and amounts based on payment mode
                            payment_mode = trans.get('payment_mode', '')
                            raw_row = trans.get('raw_row', [])
                            original_line = trans.get('original_line', '')

                            if payment_mode == 'METROBANK' and original_line:
                                # For METROBANK, extract amount and date from the line
                                amount_match = re.search(r'(\d{11,12})[A-Z]', original_line)
                                if amount_match:
                                    amount_str = amount_match.group(1)
                                    amount = float(amount_str) / 100
                                    group_total += amount  # Add to group total
                                    logger.debug(f"Found METROBANK amount: {amount} from {amount_str}")

                                # Date format: 6 digits (MMDDYY) at the end of the line
                                date_match = re.search(r'(\d{6})\d*$', original_line)
                                if date_match:
                                    date_str = date_match.group(1)
                                    # Format the date from MMDDYY to MM/DD/YY
                                ...
                                # For UNIONBANK, extract date from the line
                            ...
                            elif payment_mode == 'BDO' and len(raw_row) > 2:
            ...
            for atm_ref, transactions in processed_data.items():
                ...
                with open(report_path, 'w', encoding='utf-8') as f:
                    # Write header with ATM reference and total
                    f.write(f'ATM Reference: {atm_ref}\n')

                    # Calculate total for this ATM reference
                    atm_total = 0.0
                    for trans in transactions:
                        if isinstance(trans, dict):
                            payment_mode = trans.get('payment_mode', '')
                    ...
                    f.write(f'Total Amount: ₱{atm_total:,.2f}\n\n')
                    ...
                    f.write(f'\nTotal Amount for ATM {atm_ref}: ₱{atm_total:,.2f}\n')
                ...
                if len(line) >= 200:  # Check if line is long enough to contain ATM ref
                    ...
                    if line not in grouped_data[current_atm_ref]['raw_contents']:
                        try:
                            ...
                            amount_match = re.search(r'(\d{12})DB\d+\s*$', line)
                            if amount_match:
                                amount_str = amount_match.group(1)  # Get the first 12 digits
                                amount = float(
                                    amount_str) / 100  # Convert to float and divide by 100 for decimal points
